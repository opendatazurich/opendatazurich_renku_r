...
def create_rprofile(table_data_filenames, geo_data_filenames):
    ...
    without an active project, so project-local .Rprofile is never loaded.
    Selection order:
      1) explicit OPEN_RMD env var (filename or absolute path)
      2) the .Rmd that matches the RESOURCE_ID env var
      3) the first generated .Rmd file
    """
    ...
    if resource_id and resource_id != "NONE":
        ...
        if match:
            default_file = match[0]
    if not default_file:
        default_file = all_filenames[0]

    default_file_abs = os.path.abspath(default_file)
    default_file_r = default_file_abs.replace("\\", "/").replace('"', '\\"')

    rprofile = f'''# Auto-generated by generate_starter_rmd.py
# Opens a default .Rmd file when RStudio starts a new session.
# Override at runtime by setting OPEN_RMD to a filename or absolute path.
setHook("rstudio.sessionInit", function(newSession) {{
  if (!newSession) return(invisible(NULL))
  f <- Sys.getenv("OPEN_RMD", unset = "{default_file_r}")
  if (nzchar(f) && file.exists(f) && requireNamespace("rstudioapi", quietly = TRUE)) {{
    try(rstudioapi::navigateToFile(f), silent = TRUE)
  }}
}}, action = "append")
'''

    home = os.path.expanduser("~")
    target = os.path.join(home, ".Rprofile")
    with open(target, "w", encoding="utf-8") as file:
        file.write(rprofile)
    print(f"Wrote {target} with default file: {default_file_abs}")
