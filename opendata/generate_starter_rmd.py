...
def create_rprofile(table_data_filenames, geo_data_filenames):
    """
    Write an .Rprofile that opens a default .Rmd file when an RStudio
    session starts. Selection order:
      1) explicit OPEN_RMD env var (filename)
    ...
    default_file_r = default_file.replace("\\", "\\\\").replace('"', '\\"')

    rprofile = f'''# Auto-generated by generate_starter_rmd.py
# Opens a default .Rmd file when RStudio starts a new session.
# Override at runtime by setting the OPEN_RMD environment variable
# to a filename in the working directory.
...
'''
    ...
    with open(".Rprofile", "w", encoding="utf-8") as file:
        file.write(rprofile)
    print(f"Wrote .Rprofile with default file: {default_file}")
